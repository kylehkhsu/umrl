...
class Point2DTrajectoryEnv(Point2DEnv):
    def __init__(self,
                 **kwargs
                 ):
        self.quick_init(locals())
    ...
    def reset(self):
        self._step = 0
        if self.fixed_goal:
            self._target_position = np.array([2, 2])
        else:
            self._target_position = np.random.uniform(
                size=2, low=-self.max_target_distance/2, high=self.max_target_distance/2
            )
        if self.randomize_position_on_reset:
            self._position = np.random.uniform(
                size=2, low=-self.boundary_dist, high=self.boundary_dist
    ...
    def step(self, velocities):
        ...
        distance_to_target = np.linalg.norm(
            np.mean(self._trajectory, axis=0) - self._target_position
        )
        is_success = distance_to_target < self.target_radius
    ...
    def compute_rewards(self, actions, obs):
        achieved_goals = np.mean(self._trajectory, axis=0)
        desired_goals = obs['state_desired_goal']
        d = np.linalg.norm(achieved_goals - desired_goals, axis=-1)
        if self.reward_type == "sparse":
            return -(d > self.target_radius).astype(np.float32)
