...
class VecPyTorch(VecEnvWrapper):
    ...
    def extract_obs(self, obs):
        if type(obs) is dict:
            # obs = np.array(np.concatenate((obs['observation'], obs['desired_goal']), axis=1))
            obs = obs['observation']
        elif obs.dtype == 'O':
            # obs = np.array([np.concatenate((ob['observation'], ob['desired_goal'])) for ob in obs])
            obs = np.array([ob['observation'] for ob in obs])
        return obs
