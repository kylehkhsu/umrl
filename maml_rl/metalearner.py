...
class MetaLearner(object):
    ...
    def __init__(self, sampler, policy, baseline, gamma=0.95,
                 fast_lr=0.5, tau=1.0, entropy_coef=0, device='cpu'):
        ...
        self.to(device)
        self.entropy_pi_pre = None
        self.entropy_pi_post = None

    def reset(self):
        self.entropy_pi_pre = []
        self.entropy_pi_post = []
    ...
    def inner_loss(self, episodes, params=None):
        ...
        self.entropy_pi_pre.append(entropy.item())
        loss = -weighted_mean(log_probs * advantages, dim=0,
            weights=episodes.mask) - self.entropy_coef * entropy
    ...
    def surrogate_loss(self, episodes, old_pis=None):
        ...
        for (train_episodes, valid_episodes), old_pi in zip(episodes, old_pis):
            ...
            with torch.set_grad_enabled(old_pi is None):
                ...
                entropy = pi.entropy().mean()
                self.entropy_pi_post.append(entropy.item())

                loss = -weighted_mean(ratio * advantages, dim=0,
                    weights=valid_episodes.mask) - self.entropy_coef * entropy
                losses.append(loss)
