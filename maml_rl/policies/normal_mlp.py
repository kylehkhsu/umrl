...
class NormalMLPPolicy(Policy):
    ...
    def __init__(self, input_size, output_size, hidden_sizes=(),
                 bias_transformation_size=0, init_gain=1, nonlinearity=F.relu, init_std=1.0, min_std=1e-6):
        ...
        layer_sizes = (input_size + bias_transformation_size,) + hidden_sizes
    ...
    def forward(self, input, params=None):
        ...
        output = torch.cat([input, params['bias_transformation'].expand(*batch_shape, -1)], dim=-1)
        for i in range(1, self.num_layers):
